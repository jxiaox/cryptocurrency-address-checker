...
describe('validate()', () => {
  ...
  const mainnetChecker = new DASHChecker();
  it('should validated when use correct dash address', () => {
    expect(mainnetChecker.validate('Xx4dYKgz3Zcv6kheaqog3fynaKWjbahb6b')).toBe(
      true
    );
    expect(mainnetChecker.validate('XcY4WJ6Z2Q8w7vcYER1JypC8s2oa3SQ1b1')).toBe(
      true
    );
    expect(mainnetChecker.validate('XqMkVUZnqe3w4xvgdZRtZoe7gMitDudGs4')).toBe(
      true
    );
    expect(mainnetChecker.validate('XxN85DoLW6dTa21f8RmUuUx1zyBCMESPMw')).toBe(
      true
    );
  });
  ...
  it('should failed when use wrong dash address', () => {
    ...
    expect(mainnetChecker.validate('Xx4dYKgz3Zcv6kheaqog3fynaKWjbahabc')).toBe(
      false
    );
    // invalid base58 string
    expect(mainnetChecker.validate('Xx4dYKgz3Zcv6kheaqog3fynaKWjbaha553')).toBe(
      false
    );
    expect(mainnetChecker.validate('X1234')).toBe(false);
    expect(mainnetChecker.validate('X')).toBe(false);
  });
});

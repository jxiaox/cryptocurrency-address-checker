...
class NEOChecker implements IChecker, ICoin {
  public name: string;
  ...
  constructor(networkType: Network_type = Network_type.Mainnet) {
    ...
    this.hashAlgorithm = coinsConfig.neo.algorithm;
    this.name = coinsConfig.neo.fullName;
    this.symbol = coinsConfig.neo.symbol;
  }
  ...
  public preCheck(address: string): boolean {
    return coinsConfig.neo.addressReg.some(reg => reg.test(address));
  }

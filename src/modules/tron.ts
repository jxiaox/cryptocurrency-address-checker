...
class TRXChecker implements IChecker, ICoin {
  public name: string;
  ...
  constructor(networkType: Network_type = Network_type.Mainnet) {
    ...
    this.hashAlgorithm = coinsConfig.trx.algorithm;
    this.name = coinsConfig.trx.fullName;
    this.symbol = coinsConfig.trx.symbol;
  }
  ...
  public preCheck(address: string): boolean {
    return coinsConfig.trx.addressReg.some(reg => reg.test(address));
  }

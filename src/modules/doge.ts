...
class DogeChecker implements IChecker, ICoin {
  public name: string;
  ...
  constructor(networkType: Network_type = Network_type.Mainnet) {
    ...
    this.hashAlgorithm = coinsConfig.doge.algorithm;
    this.name = coinsConfig.doge.fullName;
    this.symbol = coinsConfig.doge.symbol;
  }
  ...
  public preCheck(address: string): boolean {
    return coinsConfig.doge.addressReg.some(reg => reg.test(address));
  }
